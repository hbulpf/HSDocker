...
但上面这种方式还不够高效。如果我们以增量的方式来计算就更加高效，例如，计算t+4秒这个时刻过去5秒窗口的WordCount，那么我们可以将t+3时刻过去5秒的统计量加上[t+3，t+4]的统计量，在减去[t-2，t-1]的统计量，这种方法可以复用中间三秒的统计量，提高统计的效率。  
...
下面介绍使用Kafka和Spark Streaming搭建实时流量统计框架。  
...
下载完成后，在 `src/main/java` 的目录下，点击java目录新建一个package命名为`spark.streaming.test` ，然后在包下新建一个  `SparkStreaming` 的java class。  
...
点击 File -> Project Structure -> Aritifacts –>点击加号 -> JAR -> from modules with dependences -> 选择刚才新建的module –> 选择Main Class -> Ok -> 选择Output layer –>点击Ok。**去掉除 `guava-14.0.1.jar` 和 `guice-3.0.jar` 以外所有的JAR包**，点击Ok。    

![图](./images/ex18/Screenshot%20from%202018-07-24%2015-54-24.png)    


![图](./images/ex18/Screenshot%20from%202018-07-24%2015-54-48.png)  

点击Build -> Build Aritifacts 。  将生成的jar包传至master节点。  


### 18.4.3 提交spark任务，测试结果
新建一个SSH连接到master节点，通过 `nc -lk 9999` 命令设置监听，并输入测试数据
```
...
centos 安装nc `yum install -y install nc` 

ubuntu 安装nc `apt-get -y install netcat-traditional`
