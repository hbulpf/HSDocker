...
<configuration>
    <property>
        ...
        <description> 节点管理器运行的附加服务 </description>
    </property>
    <property>
    ...
    <property>
        ...
        <value>master</value>
        <description> ResourceManager 的主机名</description>
    </property>
</configuration>
```
yarn-site.xml是YARN守护进程的配置文件。

完成配置后通过scp命令复制到各个slave节点即可。

### 4.4.2 统一启动YARN
确认已经配置 slaves 文件，启动HDFS后，运行yarn启动脚本
```
root@master:# /usr/local/hadoop/sbinstart-yarn.sh
```
...
### 4.4.4 查看Web界面
若是服务器或本地主机，只需要打开浏览器输入 master 的IP和端口号(一般为8088), 即可在Web界面上看到YARN相关信息。在但里集群使用的是容器无法实现，所以我们可以使用端口映射：

```
ykk@ykk-TN15S:~/team/docker-hadoop/hadoop-cluster-docker$ sudo docker inspect master | grep IPAddress 
...
映射哪个端口就指定哪个,这里映射为 8088 ，之后打开浏览器输入 localhost:8088 即可在Web界面看到 YARN 的相关信息。

### 4.4.5 提交DistributedShell任务
distributedshell，可以看做YARN编程中的 “hello world” ，它的主要功能是并行执行用户提供的shell命令或者shell脚本。 

`-jar` 指定了包含ApplicationMaster的jar文件，`-shell_command` 指定了需要被ApplicationMaster执行的Shell命令。
...
```
然后将 master 的 `/usr/local/hadoop/etc/hadoop/mapred-site.xml` 文件拷贝到各个Slave节点，重启集群。
...
+ 第二个参数是指每个map任务，取样的个数。两数相乘即为总的取样数。

Pi Estimator使用Monte Carlo方法计算Pi值的。

## 4.5 实验结果
(1) yarn启动之后在 localhost:8088 的web界面上能看到的界面:
...
Starting Job
18/06/23 09:45:57 INFO client.RMProxy: Connecting to ResourceManager at master/172.19.0.2:8032
...
18/06/23 09:45:58 INFO mapreduce.Job: The url to track the job: http://master:8088/proxy/application_1529724645512_0002/
